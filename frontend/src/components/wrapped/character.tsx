...
export function CharacterMatchSlide({ userStats }: { userStats: UserStats }) {
  const energy = userStats.characterAudioFeatures?.avgEnergy ?? 0
  const danceability = userStats.characterAudioFeatures?.avgDanceability ?? 0
  const valence = userStats.characterAudioFeatures?.avgValence ?? 0
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <Progress value={energy * 100} className="w-20" />
                <span className="text-sm">{Math.round(energy * 100)}%</span>
              </div>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm">Positivity Vibe</span>
              <div className="flex items-center gap-2">
                <Progress value={valence * 100} className="w-20" />
                <span className="text-sm">{Math.round(valence * 100)}%</span>
              </div>
            </div>
            <div className="flex justify-between items-center">
                <span className="text-sm">Ability to dance it out</span>
                <div className="flex items-center gap-2">
                  <Progress value={danceability * 100} className="w-20" />
                  <span className="text-sm">{Math.round(danceability * 100)}%</span>
                </div>
              </div>
          </div>
